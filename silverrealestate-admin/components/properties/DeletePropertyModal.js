...
export default function DeletePropertyModal({ isOpen, onClose, property, onSuccess }) {
  ...
  const handleDelete = async () => {
    setIsLoading(true);
    try {
      const res = await api.delete(`/admin/property/delete/${property.id}`);
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative bg-white rounded-2xl shadow-xl w-full max-w-sm overflow-hidden flex flex-col p-6 text-center">
        ...
        <h3 className="type-h3 font-semibold text-gray-900 mb-2">Delete Property</h3>
        <p className="type-body-sm text-gray-500 mb-6">
          Are you sure you want to delete{" "}
          <span className="font-semibold text-gray-800">{property.title || "this property"}</span>? This action cannot be undone.
        </p>
